...
podptext = 'Привет друзья! Наш бот абсолютно бесплатен и без рекламы! Но доступ у него открыт только подписчикам нашего канала👉 '+ '[ФИЛЬМЫ И СЕРИАЛЫ]'+'(https://t.me/joinchat/AAAAAEG2kXz57ViO_tZXyA)' \
            "\n" \
            "\n" \
...
def send_welcome(message):
    ...
    try:
        ...
        mydb.close()
        chri = "member"
        if chri == bot.get_chat_member(chat_id="@movies_filmy", user_id=message.from_user.id).status:
...
def callback_inline(call):
    ...
    if call.message:
        if call.data == 'testp':
            chri = "member"
            if chri == bot.get_chat_member(chat_id="@movies_filmy", user_id = call.message.chat.id).status:
                            ...
                            if len(data) <= 0:
                                ...
                                for i in data:
                                    if i['type'] == 'movie' and i['kinopoisk_id'] != 1049459 and i['kinopoisk_id'] != 989978 and i['kinopoisk_id'] != 1055319 and i['kinopoisk_id'] != 1183571 and i['kinopoisk_id'] != 1201533 and i['kinopoisk_id'] != 1102923 and i['kinopoisk_id'] != 809823 and i['kinopoisk_id'] != 1245524 and i['kinopoisk_id'] != 995075 and i['kinopoisk_id'] != 1281638 and i['kinopoisk_id'] != 737900 and i['kinopoisk_id'] != 1112920 and i['kinopoisk_id'] != 1371715:
                                        ...
                                        url3 = "https://t.me/joinchat/AAAAAEG2kXz57ViO_tZXyA"                   
                                        keyboard = types.InlineKeyboardMarkup()
